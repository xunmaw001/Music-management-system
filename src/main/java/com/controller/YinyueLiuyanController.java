...
public class YinyueLiuyanController {
    ...
    */
    ...
    public R update(@RequestBody YinyueLiuyanEntity yinyueLiuyan, HttpServletRequest request){
...
//            yinyueLiuyan.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        //根据字段查询是否有相同数据
        Wrapper<YinyueLiuyanEntity> queryWrapper = new EntityWrapper<YinyueLiuyanEntity>()
            .eq("id",0)
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        YinyueLiuyanEntity yinyueLiuyanEntity = yinyueLiuyanService.selectOne(queryWrapper);
        yinyueLiuyan.setUpdateTime(new Date());
        if(yinyueLiuyanEntity==null){
...
//                            yinyueLiuyanEntity.setYinyueLiuyanText(data.get(0));                    //留言内容 要改的
